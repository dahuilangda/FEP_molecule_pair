...
def generate_conformers(mol: Chem.rdchem.Mol,
    ...
    core_with_wildcards = AllChem.ReplaceSidechains(ref_mol, ref_smi)
    ...
    for i in range(num_conf):
        ...
        temp_mol = Chem.MolFromSmiles(Chem.MolToSmiles(mol_wh))
        AllChem.EmbedMolecule(temp_mol)
        AllChem.UFFOptimizeMolecule(temp_mol)
...
if __name__ == "__main__":
    ...
    suppl = Chem.SmilesMolSupplier(smiles_file_name, titleLine=True)
    ...
    smiles_list = []
    names = []
    for step, mol in enumerate(suppl):
        try:
            smiles_list.append(Chem.MolToSmiles(mol))
            names.append('CPD-gdc2002-%s' %str(step+1))
        except:
            pass
    added = p_map(p_run, smiles_list, names)
